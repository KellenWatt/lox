...
static void defineNative(const char* name, NativeFn function) {
    VM_push(OBJ_VAL(copyString(name, (int)strlen(name))));
    VM_push(OBJ_VAL(newNative(function)));
...
void VM_init() {
    resetStack();
    vm.objects = NULL; // calling this on an already initialized vm may cause memory leaks
    vm.bytesAllocated = 0;
    vm.nextGC = 1024 * 1024;
...
static bool call(ObjClosure* closure, int argCount) {
    if(argCount != closure->function->arity) {
        runtimeError("Expected %d arguments byt got %d", closure->function->arity, argCount);
        return false;
    }
...
#define BINARY_OP(valueType, op) \
    ...
    for (;;) {
        ...
        case OP_NOT: VM_push(BOOL_VAL(isFalsey(VM_pop()))); break;
        case OP_NEGATE: {
            if (!IS_NUMBER(peek(0))) {
        ...
        case OP_CLASS:
            VM_push(OBJ_VAL(newClass(READ_STRING())));
            break;
        case OP_INHERIT: {
...
/*     Compiler_compile(source); // May need to adjust this later for type-related stuff */
/*     return INTERPRET_OK; */
/*     [> vm.chunk = chunk; <] */
/*     [> vm.ip = vm.chunk->code; <] */
/*     [> return run(vm); <] */
